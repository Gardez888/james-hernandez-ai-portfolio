# james-hernandez-ai-portfolio **Full Resume**: [James Hernandez Resume](Resume-James-Hernandez.md) (or whatever filename)
Built by James Hernandez – Warehouse data specialist (15+ years accuracy/resolution) self-studying AI/ML for entry-level remote annotation/evaluation roles.
Portfolio of analytical writing and evaluation samples demonstrating skills in detail-oriented observation, clear communication, and pattern recognition for AI data annotation/training roles.
Detail-oriented data specialist transitioning to AI data annotation, evaluation, and training roles.

**Transferable Strengths**:
- 15+ years high-accuracy data management & discrepancy resolution
- Mentoring & clear instruction delivery (trained 50+ team members)
- Analytical observation & pattern recognition
- Self-studying Python, AI/ML basics (Coursera/edX)

## Analytical Writing & Evaluation Samples

### Gordon Solie Commentary Analysis
Detailed breakdown of legendary wrestling announcer Gordon Solie's vocal delivery, interview style, and narrative control. Demonstrates:
- Precision in observing subtle patterns (tone shifts, "uh" as collection pause)
- Clear, structured descriptive writing
- Objective evaluation of communication effectiveness

This sample highlights skills directly applicable to evaluating AI-generated text for nuance, coherence, tone consistency, and guideline adherence in RLHF/annotation tasks.
**File**: [Gordon Solie Commentary Analysis](Gordon-Solie-Analysis.md)
